...
import { Phone, Users, TrendingUp, Clock, LogOut } from 'lucide-react';
      ...
      <div className="p-6 max-w-7xl mx-auto">
        ...
        <div className="grid grid-cols-4 gap-4 mb-6">
          {[
            ...
            { label: 'Tasa contacto', value: `${calls.length > 0 ? Math.round((completed / calls.length) * 100) : 0}%`, icon: Users, color: 'text-purple-400' },
                ...
                {calls.map((call) => (
                  <tr key={call.id} className="border-b border-gray-800 hover:bg-gray-800/50 transition-colors">
                    ...
                    <td className="px-6 py-3">
                      <span className={`text-xs px-2 py-1 rounded-md border ${statusBadge(call.status)}`}>
                        {call.status}
                      </span>
                    </td>
