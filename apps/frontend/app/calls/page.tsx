...
export default function CallsPage() {
  ...
  const playRecording = (callId: string, url: string) => {
    if (playing === callId) { setPlaying(null); return; }
    setPlaying(callId);
        ...
        <div className="flex gap-2 mb-6 flex-wrap">
          {[
            ...
            <button key={f.key} onClick={() => setFilter(f.key)}
              className={`text-xs px-3 py-1.5 rounded-lg border transition-colors ${filter === f.key ? 'bg-blue-600 border-blue-600 text-white' : 'bg-gray-900 border-gray-700 text-gray-400 hover:border-gray-500'}`}>
              {f.label}
              <span className="ml-1.5 opacity-60">
                {f.key === 'all' ? calls.length : f.key === 'completed' ? calls.filter(c => c.status === 'completed').length : calls.filter(c => c.result === f.key).length}
              </span>
            </button>
          ))}
        </div>
              ...
              <div key={call.id} className="bg-gray-900 border border-gray-800 rounded-xl overflow-hidden">
                <div className="p-4 flex items-center justify-between">
                  ...
                  <div className="flex items-center gap-3 flex-wrap justify-end">
                    ...
                    <div className="text-xs text-gray-500">{new Date(call.created_at).toLocaleString('es-MX')}</div>
                    {call.recording_url && (
                      <button
                        onClick={() => playRecording(call.id, call.recording_url)}
                        className={`flex items-center gap-1.5 text-xs px-3 py-1.5 rounded-lg border transition-colors ${playing === call.id ? 'bg-green-950 text-green-400 border-green-800' : 'bg-gray-800 text-gray-300 border-gray-700 hover:border-gray-500'}`}
                      >
                        <Play size={10} />
                    ...
                    {call.summary && call.summary.length > 10 && (
                      <button onClick={() => setExpanded(expanded === call.id ? null : call.id)} className="flex items-center gap-1 text-xs text-blue-400 hover:text-blue-300 transition-colors">
